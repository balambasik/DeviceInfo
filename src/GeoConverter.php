...
class GeoConverter
{
    ...
    private static $name = [
        'Unknown country', 'Afghanistan', 'Åland Islands',
    ...
    public static function getAlfa2List()
    {
        return self::$alfa2;
    }

    public static function getCountryNamesList()
    {
        return self::$name;
    }
    ...
    public function convert()
    {
        if (is_numeric($this->geocode)) {
            ...
            $key = array_search($this->geocode, self::$alfa2);
        } elseif (mb_strlen($this->geocode, "UTF-8") == 3) {
            $key = array_search($this->geocode, $this->alfa3);
        } else {
            $key = array_search($this->geocode, self::$name);
        }

        if ($key) {
            return [
                'name'    => self::$name[$key],
                'alfa2'   => self::$alfa2[$key],
